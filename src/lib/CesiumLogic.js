export function initCesiumMap(containerId, callbacks) {
  ...
  function chooseBestSentinelItem(items) {
    // The product requirement is recency-first. Cloud cover remains visible
    // in the UI as a quality warning but no longer causes an older scene to win.
    return (items || [])
      .filter((item) => Number.isFinite(new Date(item.properties?.datetime).getTime()))
      .sort((a, b) => new Date(b.properties.datetime) - new Date(a.properties.datetime))[0] || null;
  }
